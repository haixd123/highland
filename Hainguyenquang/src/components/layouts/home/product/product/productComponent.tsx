...
const Product: any = () => {
  ...
  //! Dùng reducer để tính tổng giá trị đơn hàng
  ...
  // let sumCart = 0
  // const testSum1 = () => {
  //   const testSum = dataCart.map((record: any) => {
  //     if (record.quantity) {
  //       record.quantity += 1
  //       console.log('record.quantity1: ', record.quantity);
  //       return ((1 - (parseFloat(record.discount) / 100)) * Number(record.price.replace(/₫|,/g, '')) * record.quantity)
  //     }
  //     console.log('record.quantity2: ', record.quantity);
  //     return ((1 - (parseFloat(record.discount) / 100)) * Number(record.price.replace(/₫|,/g, '')) * record.quantity)
  //   })
  //   for (var element of testSum) {
  //     sumCart += element
  //   }
