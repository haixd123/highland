  ...
  const onFinish = async (values: any) => {
    ...
    if (status === STATUS.CREATE) {
      ...
      const response2 = await axios.put(
        `http://localhost:8888/account/${values.AccountID}`,
        values
      );
  ...
  return (
    <Layout>
      ...
      <Layout hasSider>
        ...
        >
          <div style={{ margin: "15px auto" }}>Product</div>
          <HeaderMenu />
          <div style={{ margin: "15px auto" }}>Category</div>
          <HeaderMenu />
        </Sider>
        <Content>
          ...
          <Input.Search
            onSearch={(value, event) => {
              ...
              const newData = data.filter(
                (record: any) => record?.AccountID === parseInt(value)
              );
              setNewData(newData);
              if (!show) {
